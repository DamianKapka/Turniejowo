...
namespace Turniejowo.API.Services
{
    public class MatchService : IMatchService
    {
        ...
        {
            Match correspondingBracketMatch;

            if (match.BracketIndex % 2 == 0)
            {
                correspondingBracketMatch = await matchRepository.FindSingleAsync(m => m.BracketIndex == match.BracketIndex - 1);
            }
            else
            {
                correspondingBracketMatch = await matchRepository.FindSingleAsync(m => m.BracketIndex == match.BracketIndex + 1);
            ...
            {
                var generatedMatch = await bracketManager.AutoGenerateBracketMatchAsync(correspondingBracketMatch,match);
